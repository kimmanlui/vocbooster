...
<?php
 ...
 $usedView_word="v_student_performance_wk";
 ?>
<head>
 ...
 //   require("inc/getdeckname.inc");
    ...
    try {
        ...
        $tableCols = array();
            ...
            {
              $sql = "
                     select @rownum:=@rownum+1 No, word, learning from $usedView_word , (SELECT @rownum:=0) r   where learning>=1 and userid=$id and wk=$wk 
                     order by word; 
                     ";    
            }
            ...
            {
              $sql = "
                     select @rownum:=@rownum+1 No, word, learning from $usedView_word , (SELECT @rownum:=0) r  where learning>=$learningThreshold and learning<1  and userid=$id and wk=$wk 
            ...
            {
              $sql = "
                     select @rownum:=@rownum+1 No, word, learning from $usedView_word , (SELECT @rownum:=0) r  where  learning<$learningThreshold and userid=$id and wk=$wk 
                     order by learning, word; 
                     ";    
            }       
