...
$username=$_SESSION['name']; 
    ...
    try {
      ...
      $checksql="select 'A' from decks where enable=1 and deckid in (select distinct deckid from flashcards)";
      $stmt = $conn->prepare("$checksql");  
      $stmt->execute();
      
      $count = $stmt->rowCount();
      
      $view="quiz_eg_back_v_easy";
      if ($count==1) $view="quiz_eg_back_v";

      //$conditionV="(67, 68, 70, 74, 86)";
        
        $sql = "
            select deckid, q, ans, R1, R2, R3, R4, R5 from $view where wordcount(ans)=1
              order by rand() limit $numLearnByExample
            ";
...
echo "$title";
echo "<div id=fo>";
echo "Select the BEST answer:<BR><BR>";
echo "<form method='post' action='learndict_egData.php' >";
